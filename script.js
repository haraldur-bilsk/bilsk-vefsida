...
function header(){
  ...
  if(isHome){
    return `<header class="header"><button class="menu-btn" onclick="openMenu()">☰</button><a class="phone" href="tel:+3545600000">+354 560 0000</a></header><div class="overlay" onclick="closeMenu()"></div><aside class="drawer"><div class="drawer-head"><img class="drawer-logo" src="bilskurinn-logo.png"><button class="close-btn" onclick="closeMenu()">×</button></div><nav><a href="cars">${t('nav.cars')}</a><a href="sell">${t('nav.sellDrawer')}</a><a href="import">${t('nav.import')}</a><a href="about">${t('nav.about')}</a><a href="#contact">${t('nav.contact')}</a></nav>${langSwitchHtml().replace('lang-switch','lang-switch lang-switch-drawer')}</aside>`;
  }
...
function preloadLogos(){
  ['bilskurinn-logo.png','bilskurin-logo-light.png'].forEach(src=>{const img=new Image();img.src=src});
}
