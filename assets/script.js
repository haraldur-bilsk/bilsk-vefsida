...
function carCard(car){return `<a class="card reveal" href="car?id=${car.id}"><div style="overflow:hidden"><img class="car-img${hasPhoto(car)?'':' no-photo'}" src="${carThumb(car)}" alt="${car.name}"></div><div class="card-body">${car.inStock?'<span class="stock-badge">'+t('filter.inStock')+'</span>':''}<h3 class="car-title">${car.name}</h3><div class="meta"><span>${car.year} ${t('card.yearSuffix')}</span><span>${km(car.km)}</span><span>${tFuel(car.fuel)}</span><span>${tGear(car.gear)}</span></div><div class="price">${kr(car.price)}</div></div></a>`}
