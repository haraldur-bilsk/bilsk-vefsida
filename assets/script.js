...
// Ef bakendinn er ekki tiltækur er listinn einfaldlega tómur - vefurinn á
// aldrei að sýna sýnigögn/demo-bíla, hvorki að staðaldri né sem fallback.
async function loadCars(){
  try{
    ...
    console.warn('Gat ekki sótt bíla frá /api/cars - sýni engan bíl:', err.message);
    CARS = [];
  }
  applyTranslations();
}
let CARS = [];
// Sýnigögn frá fyrri frumgerð - EKKI notuð lengur (hvorki að staðaldri né
// sem fallback). Vefurinn á aðeins að sýna alvöru bíla frá Henry/Rögg API-inu.
...
function carColor(car){return car.color||'-'}
...
const I18N={
  is:{
    ...
    'car.eyebrow':'Bíll til sölu','car.title':'Bíladetail','car.inquiryTitle':'Fá frekari upplýsingar','car.form.notice':'Takk! Fyrirspurnin hefur verið móttekin.','car.form.submit':'Senda fyrirspurn',
    'cars.eyebrow':'Bílasafn','cars.title':'Bílar til sölu','cars.lead':'Skoðaðu alla bíla sem eru til sölu hjá Bílskúrnum.',
    ...
    'sell.form.notice':'Takk! Beiðnin hefur verið móttekin.','sell.form.submit':'Senda bíl í sölu',
  ...
  en:{
    ...
    'car.eyebrow':'Car for sale','car.title':'Car details','car.inquiryTitle':'Get more information','car.form.notice':'Thanks! Your inquiry has been received.','car.form.submit':'Send inquiry',
    'cars.eyebrow':'Car collection','cars.title':'Cars for sale','cars.lead':'Browse all the cars for sale at Bílskúrinn.',
    ...
    'sell.form.notice':'Thanks! Your request has been received.','sell.form.submit':'Submit car for sale',
  ...
  pl:{
    ...
    'car.eyebrow':'Samochód na sprzedaż','car.title':'Szczegóły samochodu','car.inquiryTitle':'Uzyskaj więcej informacji','car.form.notice':'Dziękujemy! Zapytanie zostało odebrane.','car.form.submit':'Wyślij zapytanie',
    'cars.eyebrow':'Kolekcja samochodów','cars.title':'Samochody na sprzedaż','cars.lead':'Przeglądaj wszystkie samochody na sprzedaż w Bílskúrinn.',
    ...
    'sell.form.notice':'Dziękujemy! Zgłoszenie zostało odebrane.','sell.form.submit':'Zgłoś samochód do sprzedaży',
...
function header(){
  const path=(location.pathname.split('/').pop()||'').toLowerCase().replace(/\.html$/,'');
  const isHome=path===''||path==='index';
  if(isHome){
    return `<header class="header"><button class="menu-btn" onclick="openMenu()">☰</button><a class="phone" href="tel:+3545600000">+354 560 0000</a></header><div class="overlay" onclick="closeMenu()"></div><aside class="drawer"><div class="drawer-head"><img class="drawer-logo" src="assets/bilskurinn-logo.png"><button class="close-btn" onclick="closeMenu()">×</button></div><nav><a href="cars">${t('nav.cars')}</a><a href="sell">${t('nav.sellDrawer')}</a><a href="import">${t('nav.import')}</a><a href="about">${t('nav.about')}</a><a href="#contact">${t('nav.contact')}</a></nav>${langSwitchHtml().replace('lang-switch','lang-switch lang-switch-drawer')}</aside>`;
  }
  const currentKey=(path==='cars'||path==='car')?'cars':path==='sell'?'sell':path==='import'?'import':path==='about'?'about':'';
  const navItems=[
    {key:'cars',href:'cars',label:t('nav.cars')},
    {key:'sell',href:'sell',label:t('nav.sellTop')},
    {key:'import',href:'import',label:t('nav.import')},
    {key:'about',href:'about',label:t('nav.about')},
    {key:'contact',href:'/#contact',label:t('nav.contact')}
  ];
  return `<header class="header header-nav"><a class="header-logo-link" href="/"><img class="header-logo-inline" src="assets/bilskurinn-logo.png" alt="Bílskúrinn"></a><nav class="header-nav-links">${navItems.map(n=>`<a href="${n.href}"${n.key===currentKey?' class="active"':''}>${n.label}</a>`).join('')}</nav>${langSwitchHtml().replace('lang-switch','lang-switch lang-switch-top')}<a class="phone" href="tel:+3545600000">+354 560 0000</a></header>`;
}
...
function carMetaParts(car){const parts=[];if(car.year)parts.push(`${car.year} ${t('card.yearSuffix')}`);if(car.km)parts.push(km(car.km));if(car.fuel)parts.push(tFuel(car.fuel));if(car.gear)parts.push(tGear(car.gear));return parts}
function carCard(car){const metaHtml=carMetaParts(car).map(p=>`<span>${p}</span>`).join('');return `<a class="card reveal" href="car?id=${car.id}"><div style="overflow:hidden"><img class="car-img${hasPhoto(car)?'':' no-photo'}" src="${carThumb(car)}" alt="${car.name}"></div><div class="card-body">${car.inStock?'<span class="stock-badge">'+t('filter.inStock')+'</span>':''}<h3 class="car-title">${car.name}</h3><div class="meta">${metaHtml}</div><div class="price">${kr(car.price)}</div></div></a>`}
...
// ATH (13.07.2026): renderSpecCategories()/SPEC_CATEGORIES hér að ofan er
// ELDRI TILRAUNAGÖGN - búnaður er þar búinn til með slembitölu út frá auðkenni
// bílsins (specHash/specBool o.fl.), EKKI raunveruleg gögn. Þetta var notað
// meðan enginn alvöru búnaðarlisti var til. Núna er kominn ALVÖRU búnaðarlisti
// frá Henry/Rögg (codes.aspx -> car.features, sjá backend/server.js) svo
...
async function postFormJSON(url,data){
  try{
    ...
    console.warn('Sending mistókst ('+url+'):',err.message);
    return false;
  }
}
async function sendInquiryToServer(form){
  const car=currentDetailCar();
  const name=form.querySelector('[name="name"]')?.value||'';
  const email=form.querySelector('[name="email"]')?.value||'';
  const phone=form.querySelector('[name="phone"]')?.value||'';
  const message=form.querySelector('[name="message"]')?.value||'';
  return postFormJSON('/api/inquiry',{name,email,phone,message,carId:car?car.id:'',carName:car?car.name:''});
}
async function sendBookingToServer(form){
  const car=currentDetailCar();
  const name=form.querySelector('[name="name"]')?.value||'';
  const email=form.querySelector('[name="email"]')?.value||'';
  const phone=form.querySelector('[name="phone"]')?.value||'';
  ...
  return postFormJSON('/api/booking',{name,email,phone,date,time,carId:car?car.id:'',carName:car?car.name:''});
}
async function sendSellToServer(form){
  const val=n=>form.querySelector('[name="'+n+'"]')?.value||'';
  const fd=new FormData();
  ['fullName','email','phone','plate','make','model','year','mileage','price','extra'].forEach(n=>fd.append(n,val(n)));
  sellPhotoFiles.forEach(f=>fd.append('photos',f));
  try{
    const res=await fetch('/api/sell',{method:'POST',body:fd});
    return res.ok;
  }catch(err){
    console.warn('Sending mistókst (/api/sell):',err.message);
    return false;
  }
}
...
async function sendImportToServer(form){
  const val=n=>form.querySelector('[name="'+n+'"]')?.value||'';
  const data={fullName:val('fullName'),email:val('email'),phone:val('phone'),make:val('make'),model:val('model'),year:val('year'),budget:val('budget'),fuel:val('fuel'),mileage:val('mileage'),color:val('color'),desc:val('desc')};
  return postFormJSON('/api/import',data);
}

async function fakeSubmit(e){
  e.preventDefault();
  const form=e.target;
  ...
  let ok=true;
  try{
    if(form.id==='inquiryForm')ok=await sendInquiryToServer(form);
    else if(form.id==='bookingForm')ok=await sendBookingToServer(form);
    else if(form.id==='sellForm')ok=await sendSellToServer(form);
    else if(form.id==='importForm')ok=await sendImportToServer(form);
  }finally{
    if(submitBtn)submitBtn.disabled=false;
  }
  const okBox=form.querySelector('.notice:not(.notice-error)');
  const errBox=form.querySelector('.notice-error');
  if(ok){
    if(errBox)errBox.classList.add('hidden');
    if(okBox)okBox.classList.remove('hidden');
    form.classList.add('submitted');
    form.reset();
    calculatePayout();resetSellPhotos();positionISKSuffix('sellPrice','sellPriceSuffix');positionISKSuffix('sellMileage','sellMileageSuffix');positionISKSuffix('importBudget','importBudgetSuffix');resetImportFormWidgets();
  } else {
...
function updateLogos(){
  const isLight=document.body.classList.contains('light-theme');
  const src=isLight?'bilskurin-logo-light.png':'assets/bilskurinn-logo.png';
  document.querySelectorAll('.logo-intro-img,.drawer-logo,.footer-logo,.header-logo-inline').forEach(img=>{
    if(img.getAttribute('src')!==src) img.src=src;
  });
}
// Náum í bæði ljósa og dökka merkið strax við hleðslu svo skiptingin milli
// þeirra (sjá toggleTheme) verði ekki með sýnilegu blikki fyrsta skiptið.
function preloadLogos(){
  ['assets/bilskurinn-logo.png','bilskurin-logo-light.png'].forEach(src=>{const img=new Image();img.src=src});
}
